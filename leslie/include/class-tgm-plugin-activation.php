	...
	class TGM_Plugin_Activation {
		...
		public function init() {
			...
			$this->strings = array(
				...
				'installing'                      => __( 'Installing Plugin: %s', 'leslie' ),
				'oops'                            => __( 'Something went wrong.', 'leslie' ),
				'notice_can_install_required'     => _n_noop( 'This theme requires the following plugin: %1$s.', 'This theme requires the following plugins: %1$s.', 'leslie' ),
				'notice_can_install_recommended'  => _n_noop( 'This theme recommends the following plugin: %1$s.', 'This theme recommends the following plugins: %1$s.', 'leslie' ),
				'notice_cannot_install'           => _n_noop( 'Sorry, but you do not have the correct permissions to install the %s plugin. Contact the administrator of this site for help on getting the plugin installed.', 'Sorry, but you do not have the correct permissions to install the %s plugins. Contact the administrator of this site for help on getting the plugins installed.', 'leslie' ),
				'notice_can_activate_required'    => _n_noop( 'The following required plugin is currently inactive: %1$s.', 'The following required plugins are currently inactive: %1$s.', 'leslie' ),
				'notice_can_activate_recommended' => _n_noop( 'The following recommended plugin is currently inactive: %1$s.', 'The following recommended plugins are currently inactive: %1$s.', 'leslie' ),
				'notice_cannot_activate'          => _n_noop( 'Sorry, but you do not have the correct permissions to activate the %s plugin. Contact the administrator of this site for help on getting the plugin activated.', 'Sorry, but you do not have the correct permissions to activate the %s plugins. Contact the administrator of this site for help on getting the plugins activated.', 'leslie' ),
				'notice_ask_to_update'            => _n_noop( 'The following plugin needs to be updated to its latest version to ensure maximum compatibility with this theme: %1$s.', 'The following plugins need to be updated to their latest version to ensure maximum compatibility with this theme: %1$s.', 'leslie' ),
				'notice_cannot_update'            => _n_noop( 'Sorry, but you do not have the correct permissions to update the %s plugin. Contact the administrator of this site for help on getting the plugin updated.', 'Sorry, but you do not have the correct permissions to update the %s plugins. Contact the administrator of this site for help on getting the plugins updated.', 'leslie' ),
				'install_link'                    => _n_noop( 'Begin installing plugin', 'Begin installing plugins', 'leslie' ),
				'activate_link'                   => _n_noop( 'Begin activating plugin', 'Begin activating plugins', 'leslie' ),
				'return'                          => __( 'Return to Required Plugins Installer', 'leslie' ),
				'dashboard'                       => __( 'Return to the dashboard', 'leslie' ),
				'plugin_activated'                => __( 'Plugin activated successfully.', 'leslie' ),
				'activated_successfully'          => __( 'The following plugin was activated successfully:', 'leslie' ),
				'complete'                        => __( 'All plugins installed and activated successfully. %1$s', 'leslie' ),
				'dismiss'                         => __( 'Dismiss this notice', 'leslie' ),
			);
		...
		protected function do_plugin_install() {
			...
			if ( isset( $_GET['plugin'] ) && ( isset( $_GET['tgmpa-install'] ) && 'install-plugin' === $_GET['tgmpa-install'] ) ) {
				...
				if ( empty( $complete ) ) {
					echo '<p>', sprintf( esc_html( $this->strings['complete'] ), '<a href="' . esc_url( self_admin_url() ) . '">' . esc_html__( 'Return to the Dashboard', 'leslie' ) . '</a>' ), '</p>';
			...
			if ( '' !== $desired_slug ) {
				...
				if ( ! empty( $subdir_name ) && $subdir_name !== $desired_slug ) {
					...
					if ( true === $GLOBALS['wp_filesystem']->move( $from, $to ) ) {
						...
						return new WP_Error( 'rename_failed', esc_html__( 'The remote plugin package is does not contain a folder with the desired slug and renaming did not work.', 'leslie' ) . ' ' . esc_html__( 'Please contact the plugin provider and ask them to package their plugin according to the WordPress guidelines.', 'leslie' ), array( 'found' => $subdir_name, 'expected' => $desired_slug ) );
					}
				}
				elseif ( empty( $subdir_name ) ) {
					return new WP_Error( 'packaged_wrong', esc_html__( 'The remote plugin package consists of more than one file, but the files are not packaged in a folder.', 'leslie' ) . ' ' . esc_html__( 'Please contact the plugin provider and ask them to package their plugin according to the WordPress guidelines.', 'leslie' ), array( 'found' => $subdir_name, 'expected' => $desired_slug ) );
				}
			}
		...
		public function notices() {
			...
			if ( ! empty( $message ) ) {
				...
				foreach ( $message as $type => $plugin_groups ) {
					...
					$rendered .= '<p>' . sprintf( translate_nooped_plural( $this->strings[ $type ], $count, 'leslie' ), $imploded, $count ) . '</p>';
				}

				// Setup variables to determine if action links are needed.
				$show_install_link  = $install_link ? '<a href="' . esc_url( add_query_arg( 'page', urlencode( $this->menu ), self_admin_url( $this->parent_slug ) ) ) . '">' . translate_nooped_plural( $this->strings['install_link'], $install_link_count, 'leslie' ) . '</a>' : '';
				$show_activate_link = $activate_link ? '<a href="' . esc_url( add_query_arg( 'page', urlencode( $this->menu ), self_admin_url( $this->parent_slug ) ) ) . '">' . translate_nooped_plural( $this->strings['activate_link'], $activate_link_count, 'leslie' ) . '</a>'  : '';
	...
	class TGMPA_List_Table extends WP_List_Table {
			...
			foreach ( $this->tgmpa->plugins as $plugin ) {
				...
				if ( ! empty( $plugin['source'] ) ) {
					if ( preg_match( '|^http(s)?://|', $plugin['source'] ) ) {
						...
						$table_data[ $i ]['source'] = __( 'Private Repository', 'leslie' );
					}
					else {
						// The plugin is pre-packaged with the theme.
						$table_data[ $i ]['source'] = __( 'Pre-Packaged', 'leslie' );
					}
				}
				// The plugin is from the WordPress repository.
				else {
					$table_data[ $i ]['source'] = __( 'WordPress Repository', 'leslie' );
				}

				$table_data[ $i ]['type'] = isset( $plugin['required'] ) && $plugin['required'] ? __( 'Required', 'leslie' ) : __( 'Recommended', 'leslie' );

				if ( ! isset( $installed_plugins[ $plugin['file_path'] ] ) ) {
					$table_data[ $i ]['status'] = sprintf( '%1$s', __( 'Not Installed', 'leslie' ) );
				}
				elseif ( is_plugin_inactive( $plugin['file_path'] ) ) {
					$table_data[ $i ]['status'] = sprintf( '%1$s', __( 'Installed But Not Activated', 'leslie' ) );
				}
		...
		public function column_plugin( $item ) {
			...
			if ( ! is_plugin_active( $item['file_path'] ) && ! isset( $installed_plugins[ $item['file_path'] ] ) ) {
				...
				$actions = array(
					'install' => sprintf(
						'<a href="%1$s">' . esc_html_x( 'Install %2$s', '%2$s = plugin name in screen reader markup', 'leslie' ) . '</a>',
			...
			elseif ( is_plugin_inactive( $item['file_path'] ) ) {
				...
				$actions = array(
					'activate' => sprintf(
						'<a href="%1$s">' . esc_html_x( 'Activate %2$s', '%2$s = plugin name in screen reader markup', 'leslie' ) . '</a>',
		...
		public function column_cb( $item ) {
			...
			if ( __( 'Private Repository', 'leslie' ) === $item['source'] ) {
				// Escape external URLs
				$plugin_url = esc_url( $plugin_url );
			}
			elseif ( __( 'Pre-Packaged', 'leslie' ) === $item['source'] ) {
		...
		public function no_items() {
			...
			printf( wp_kses_post( __( 'No plugins to install or activate. <a href="%1$s">Return to the Dashboard</a>', 'leslie' ) ), esc_url( self_admin_url() ) );
			echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
		...
		public function get_columns() {
			...
			$columns = array(
				...
				'plugin' => __( 'Plugin', 'leslie' ),
				'source' => __( 'Source', 'leslie' ),
				'type'   => __( 'Type', 'leslie' ),
				'status' => __( 'Status', 'leslie' ),
			);
		...
		public function get_bulk_actions() {
			...
			$actions = array(
				'tgmpa-bulk-install'  => __( 'Install', 'leslie' ),
				'tgmpa-bulk-activate' => __( 'Activate', 'leslie' ),
			);
		...
		public function process_bulk_actions() {
			...
			if ( 'tgmpa-bulk-install' === $this->current_action() ) {
				...
				if ( empty( $plugin_installs ) ) {
					echo '<div id="message" class="error"><p>', esc_html__( 'No plugins are available to be installed at this time.', 'leslie' ), '</p></div>';
					return false;
				}
			...
			if ( 'tgmpa-bulk-activate' === $this->current_action() ) {
				...
				if ( empty( $plugins_to_activate ) ) {
					echo '<div id="message" class="error"><p>', esc_html__( 'No plugins are available to be activated at this time.', 'leslie' ), '</p></div>';
					return false;
				}
				...
				if ( is_wp_error( $activate ) ) {
					...
					printf( '<div id="message" class="updated"><p>%1$s %2$s.</p></div>', esc_html( _n( 'The following plugin was activated successfully:', 'The following plugins were activated successfully:', $count, 'leslie' ) ), wp_kses_post( $imploded ) );
				}
			...
			if ( ! class_exists( 'TGM_Bulk_Installer' ) ) {
				...
				class TGM_Bulk_Installer extends WP_Upgrader {
					...
					public function install_strings() {
						...
						$this->strings['no_package']          = __( 'Install package not available.', 'leslie' );
						$this->strings['downloading_package'] = __( 'Downloading install package from <span class="code">%s</span>&#8230;', 'leslie' );
						$this->strings['unpack_package']      = __( 'Unpacking the package&#8230;', 'leslie' );
						$this->strings['installing_package']  = __( 'Installing the plugin&#8230;', 'leslie' );
						$this->strings['process_failed']      = __( 'Plugin install failed.', 'leslie' );
						$this->strings['process_success']     = __( 'Plugin installed successfully.', 'leslie' );
					...
					public function activate_strings() {
						...
						$this->strings['activation_failed']  = __( 'Plugin activation failed.', 'leslie' );
						$this->strings['activation_success'] = __( 'Plugin activated successfully.', 'leslie' );
			...
			if ( ! class_exists( 'TGM_Bulk_Installer_Skin' ) ) {
				...
				class TGM_Bulk_Installer_Skin extends Bulk_Upgrader_Skin {
					...
					public function add_strings() {
						...
						$this->upgrader->strings['skin_update_failed_error'] = __( 'An error occurred while installing %1$s: <strong>%2$s</strong>.', 'leslie' );
						$this->upgrader->strings['skin_update_failed']       = __( 'The installation of %1$s failed.', 'leslie' );

						// Automatic activation strings.
						if ( $this->tgmpa->is_automatic ) {
							$this->upgrader->strings['skin_upgrade_start']        = __( 'The installation and activation process is starting. This process may take a while on some hosts, so please be patient.', 'leslie' );
							$this->upgrader->strings['skin_update_successful']    = __( '%1$s installed and activated successfully.', 'leslie' ) . ' <a href="#" class="hide-if-no-js" onclick="%2$s"><span>' . esc_html__( 'Show Details', 'leslie' ) . '</span><span class="hidden">' . esc_html__( 'Hide Details', 'leslie' ) . '</span>.</a>';
							$this->upgrader->strings['skin_upgrade_end']          = __( 'All installations and activations have been completed.', 'leslie' );
							$this->upgrader->strings['skin_before_update_header'] = __( 'Installing and Activating Plugin %1$s (%2$d/%3$d)', 'leslie' );
						}
						// Default installation strings.
						else {
							$this->upgrader->strings['skin_upgrade_start']        = __( 'The installation process is starting. This process may take a while on some hosts, so please be patient.', 'leslie' );
							$this->upgrader->strings['skin_update_successful']    = esc_html__( '%1$s installed successfully.', 'leslie' ) . ' <a href="#" class="hide-if-no-js" onclick="%2$s"><span>' . esc_html__( 'Show Details', 'leslie' ) . '</span><span class="hidden">' . esc_html__( 'Hide Details', 'leslie' ) . '</span>.</a>';
							$this->upgrader->strings['skin_upgrade_end']          = __( 'All installations have been completed.', 'leslie' );
							$this->upgrader->strings['skin_before_update_header'] = __( 'Installing Plugin %1$s (%2$d/%3$d)', 'leslie' );
						}
					...
					public function bulk_footer() {
						...
						if ( empty( $complete ) ) {
							echo '<p>', sprintf( esc_html( $this->tgmpa->strings['complete'] ), '<a href="' . esc_url( self_admin_url() ) . '">' . esc_html__( 'Return to the Dashboard', 'leslie' ) . '</a>' ), '</p>';
							echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
						}
